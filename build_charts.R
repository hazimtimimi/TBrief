...
output$mortality_chart <-  renderPlot({
  ...
  pdata()$epi_timeseries %>%
    ...
    scale_x_continuous(name=element_blank(), breaks = c(2015, dcyear-1)) +
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# 2. Incidence and notification 2015 - latest year
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
output$incidence_chart <-  renderPlot({
  ...
  pdata()$epi_timeseries %>%
    ...
    geom_line(mapping=aes(x=year, y=c_newinc_100k, ymin=0, colour="Notified"),
              size=1.5) +
